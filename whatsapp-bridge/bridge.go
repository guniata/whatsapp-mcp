...
func handleMessage(client *whatsmeow.Client, messageStore *MessageStore, msg *events.Message, logger waLog.Logger) {
	...
	// Prefer the phone-number identity over the anonymous LID when both are
	// known, so stored senders resolve to contact names.
	sender := msg.Info.Sender.User
	if msg.Info.Sender.Server == "lid" && msg.Info.SenderAlt.Server == "s.whatsapp.net" {
...
func downloadMedia(client *whatsmeow.Client, messageStore *MessageStore, messageID, chatJID string) (bool, string, string, string, error) {
	...
	if !strings.HasPrefix(absPath, storeDir()+string(filepath.Separator)) {
		return false, "", "", "", fmt.Errorf("refusing to write outside the store directory")
	}
...
func extractDirectPathFromURL(url string) string {
	...
	// Keep the query string: it carries WhatsApp's auth tokens (oh/oe), and
	// whatsmeow appends its own params with '&', expecting them present.
	// Stripping them produces a malformed URL and a uniform 403.
	return "/" + parts[1]
}
...
func startRESTServer(client *whatsmeow.Client, messageStore *MessageStore, port int) {
	...
	// Start the server. Loopback only: this is IPC for the local MCP server,
	...
	if client.Store.ID == nil {
		...
		for evt := range qrChan {
			if evt.Event == "code" {
				...
				qrPath := filepath.Join(storeDir(), "qr.png")
...
func handleHistorySync(client *whatsmeow.Client, messageStore *MessageStore, historySync *events.HistorySync, logger waLog.Logger) {
	...
	for _, conversation := range historySync.Data.Conversations {
		...
		if len(messages) > 0 {
			// Update chat with the newest valid message timestamp. Entries can
			// be nil/stub records; skip those instead of dropping the chat.
			timestamp := time.Time{}
			for _, m := range messages {
				if m != nil && m.Message != nil {
					if ts := m.Message.GetMessageTimestamp(); ts != 0 {
			...
			for _, msg := range messages {
				...
				if msg.Message.Key != nil {
					...
					// Group message without a participant: leave sender empty —
					// storing the group's own ID would fake attribution.
				} else if jid.Server != "g.us" {
					sender = jid.User
				}
