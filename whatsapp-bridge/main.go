...
func main() {
	...
	case "bridge":
		...
		// Before anything else, including anything that can fail: a bridge that
		// dies during startup is invisible otherwise, which is exactly when
		// knowing why matters most.
		startBridgeLog()
		runBridge()
	case "mcp":
		runMCP()
